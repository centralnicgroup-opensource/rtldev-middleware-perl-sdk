package Hexonet::Connector::Response;

use strict;
use warnings;
use Hexonet::Connector::Util;
use overload
  '%{}' => \&_as_hash_op,
  '@{}' => \&as_list,
  ;
...
sub _as_hash_op {
    ...
    my ($pkg) = caller 0;
    return $self if $pkg->isa('Hexonet::Connector::Response');
    return $self->as_hash();
}
...
sub as_hash {
    ...
    $self->{_response_hash} = Hexonet::Connector::Util::response_to_hash(
...
sub as_list_hash {
    ...
    $self->{_response_list_hash} =
      Hexonet::Connector::Util::response_to_list_hash( $self->as_hash() );
...
Hexonet::Connector::Response - package to provide functionality to deal with Backend
API reponses.
...
=head1 METHODS Hexonet::Connector::Response
