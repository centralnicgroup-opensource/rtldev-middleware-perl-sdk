...
sub _as_hash_op {
	...
	my ($pkg) = caller 0;
	return $self if $pkg->isa('HEXONET::Response');
	return $self->as_hash();
}
...
sub as_hash {
	...
	$self->{_response_hash} = HEXONET::Util::response_to_hash($self->{_response_string});
...
sub as_list_hash {
	...
	$self->{_response_list_hash} = HEXONET::Util::response_to_list_hash($self->as_hash());
